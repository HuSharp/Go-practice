...
func TestGetRoute(t *testing.T) {
	...
	if ps["name"] != "hjh" {
		t.Fatal("name should be equal to 'hjh'")
	}
...
func TestGetRoute2(t *testing.T) {
	r := newTestRouter()
	route, ps := r.getRoute("GET", "/assets/file.txt")
	ok := route.pattern == "/assets/*filepath" && ps["filepath"] == "file.txt"
	if !ok {
