...
func (server *Server) handleRequest(cc codec.Codec, req *request, sending *sync.Mutex, wg *sync.WaitGroup, timeout time.Duration) {
	defer wg.Done()
	called := make(chan struct{})	// called 信道接收到消息，代表处理没有超时，继续执行 sendResponse。
...
const (
	...
	defaultDebugPath = "/debug/geeRpc"		// 后续 DEBUG 页面预留的地址
)

// ServeHTTP 实现 handler 来应答 RPC request
func (server *Server) ServeHTTP(w http.ResponseWriter, req *http.Request) {
	...
	if err != nil {
		log.Println("[ServeHTTP] rpc hijacking failed.", req.RemoteAddr, ": ", err.Error())
		return
	}
...
// HandleHTTP register HTTP handlers
func (server *Server) HandleHTTP() {
	http.Handle(defaultRPCPath, server)
	http.Handle(defaultDebugPath, debugHTTP{server})
	log.Println("[HandleHTTP] rpc server debug path:", defaultDebugPath)
}
// HandleHTTP 是一个默认的 default server to register HTTP handlers，主要为了用户使用方便。
func HandleHTTP() {
